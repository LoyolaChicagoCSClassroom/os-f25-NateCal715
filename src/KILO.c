...
void editorSave() {
    if (E.filename == NULL) {
        editorSetStatusMessage("Save not implemented yet.");
        return;
    }

    editorSetStatusMessage("Save not implemented (read-only).");
}
    ...
    for (y = 0; y < E.screenrows; y++) {
        ...
        if (filerow >= E.numrows) {
            if (E.numrows == 0 && y == E.screenrows / 3) {
                char welcome[80];
                int welcomelen = esp_printf(welcome,
...
void editorDrawStatusBar(struct abuf *ab) {
    ...
    int len = esp_printf(status, "%.20s - %d lines %s",
        E.filename ? E.filename : "[No Name]", E.numrows,
        E.dirty ? "(modified)" : "");
    int rlen = esp_printf(rstatus, "%d/%d", E.cy + 1, E.numrows);
    if (len > E.screencols) len = E.screencols;
